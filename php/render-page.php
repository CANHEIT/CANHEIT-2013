...
# parse the URI
  ...
  switch ($p) {
    ...
    case (
      ...
      array_push($parse_functions, 'get_correct_image_urls', 'parse_links', 'get_tracks_for_session', 'check_if_session_should_have_video', 'extract_video_url_from_links');
      break;
...
# helper functions
  ...
  function extract_video_url_from_links(&$data) {
    ...
    for ($i = 0; $i < count($data['links']); $i++) {
      ...
      if (preg_match($providers_pattern, $data['links'][$i]['gb_url'])) {
        $data['video_url'] = $data['links'][$i]['gb_url'];
        unset($data['links'][$i]);
        return;
      }
    }
